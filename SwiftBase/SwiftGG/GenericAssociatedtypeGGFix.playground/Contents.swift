...
//Container协议定义了三个任何遵循该协议的类型,即容器必须提供的功能
...
//要遵守Container协议,Item类型也必须遵守Equatable协议



//在关联类型约束里使用协议
//协议可以作为它自身的要求出现
//例如有一个协议细化了Container协议添加了一个suffix(_:)方法
//suffix(_:)方法返回容器中从后往前给定数量的元素并把它们存储在一个Suffix类型的实例里
...
//Suffix拥有两个约束
//它必须遵循SuffixableContainer协议,也就是当前定义的协议
//以及它的Item类型必须是和容器里的Item类型相同
